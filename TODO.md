...
- [ ] Add new single-choice slots to `KNOWN_SLOTS` + `slotOrder`: `api`, `ai`, `ui`, `payments`. Decide ordering for the wizard's topological prompts (api/ai after framework). (The add-on categories `testing`/`tooling`/`deploy`/`email` are done — see above)
