...
- [x] `tooling` slot — landed as a single-choice **slot** (not the originally-planned add-on): the three toolchains (`eslint-prettier`, `biome`, `oxlint-oxfmt`) are mutually exclusive substitutes, so single-choice fits. Two-line `KNOWN_SLOTS`/`SLOTS` add, removed from `KNOWN_ADDONS`/`ADDON_CATEGORIES`. eslint-prettier carries per-framework adapters; biome + oxlint-oxfmt are framework-agnostic. Also dropped the stale `lint: "next lint"` from `framework-next` (removed in Next 16) so the tooling module owns `scripts.lint` cleanly. CLI flag + web card render automatically off `KNOWN_SLOTS`
