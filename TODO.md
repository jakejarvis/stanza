...
- [x] Multi-choice add-on support — landed via a `kind: "slot" | "addon"` discriminator on `Module` + a `manifest.addons` record (`Partial<Record<AddonCategoryId, StanzaAddonRecord[]>>`). Add-on categories (`testing`, `tooling`, `deploy`, `email`, `monorepo`) live in `KNOWN_ADDONS`/`ADDON_CATEGORIES`, disjoint from `KNOWN_SLOTS` so the resolver leaves them out of peer dispatch (they can still declare a one-way `peers` + framework-varying adapters). Wired through the resolver (`addonOrder`), runner (`isAddon` branches), CLI (`add <slot|category>`, `remove <slot|category> [id]`, `--testing vitest,playwright` flags, `list`), and the web builder (checkbox cards, comma-joined `?testing=…` URL). Validated by `testing-vitest` + `testing-playwright`. Chose this over slot-value-arrays / sub-slots because `deploy` has no fixed partition and the resolver needed zero changes
- [x] `tooling` slot — landed as a single-choice **slot** (not the originally-planned add-on): the three toolchains (`eslint-prettier`, `biome`, `oxlint-oxfmt`) are mutually exclusive substitutes, so single-choice fits. Added to `SLOTS`, removed from `ADDON_CATEGORIES` (`SlotId`/`KNOWN_SLOTS` now derive from `SLOTS`, so it's a one-place edit). It's **repo-scoped** (`repoScoped: true`): config files land at the repo root and scripts/devDeps merge into the root `package.json` — a third install home wired in the runner + `synthesizePackageJsons`. eslint-prettier carries per-framework adapters; biome + oxlint-oxfmt are framework-agnostic. Also dropped the stale `lint: "next lint"` from `framework-next` (removed in Next 16) so the tooling module owns the root `lint`/`format` cleanly. CLI flag + web card render automatically off the taxonomy
- [ ] Add new single-choice slots to `KNOWN_SLOTS` + `slotOrder`: `api`, `ai`, `ui`, `payments`. Decide ordering for the wizard's topological prompts (api/ai after framework). (The add-on categories `testing`/`deploy`/`email` are done — see above; `tooling` became a slot)
