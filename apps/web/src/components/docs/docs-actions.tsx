...
type DocsActionsProps = {
  /** Path to the markdown alternate of this page (e.g. `/docs/registry.md`). */
  markdownPath: string;
  /** Absolute URL of this page — embedded in AI-chat prompts. */
  pageUrl: string;
  className?: string;
};

type DocsActionsMenuItem = {
  label: string;
  href: (urls: { markdownPath: string; pageUrl: string }) => string;
  icon: (props: SVGProps<SVGSVGElement>) => ReactElement;
};

const menuItems: readonly DocsActionsMenuItem[] = [
  {
...
export function DocsActions({ markdownPath, pageUrl, className }: DocsActionsProps) {
  return (
    <ButtonGroup
      className={cn(
        ...
        <DropdownMenuContent align="end" className="w-max min-w-44">
          <DropdownMenuGroup>
            {menuItems.map((item) => {
              ...
              const href = item.href({ markdownPath, pageUrl });
              return (
                <DropdownMenuItem
                  key={item.label}
...
function getPromptUrl(baseUrl: string, url: string): string {
  ...
  const prompt = `I have questions about this documentation page for Stanza, a modular scaffolding tool for full-stack TypeScript monorepos: ${url}

Study it and let me know when you're ready for my questions.`;
