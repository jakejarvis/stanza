...
Stanza generates a clean monorepo: your app under `apps/web/` (one of
potentially many apps as the registry grows), internal packages under
`packages/`, and a `stanza.json` manifest at the root. The manifest records the
apps in the project, the modules you selected, and which files each one owns —
it's how `add` and `remove` know what's installed and what to clean up. You
generally don't edit it by hand.
