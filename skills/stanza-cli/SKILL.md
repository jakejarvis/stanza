...
- `stanza remove <category> [[@<ns>/]<id>] [--app=<id>]` removes a module. The `id` is optional for single-choice categories and required for multi-choice ones — omitting it for a multi-choice category errors non-interactively with the installed list. `--app` scopes removal in multi-app projects. The `@<ns>/` prefix is accepted but optional — `remove` matches against the stored `id`.
